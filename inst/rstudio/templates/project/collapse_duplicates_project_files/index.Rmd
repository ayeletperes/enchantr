...
params:
   ...
   c_region_column:
      ...
      value: "c_region"
   locus:
      label: "`locus`: the receptor class (`IG`, `TR`) or the loci (e.g. `IGH`, or `IGH,IGK`) the samples target. Comma-separated."
      input: text
      value: "IG"
   restrict_to_locus:
      label: "`restrict_to_locus`: Drop sequences whose locus is not one of `locus`. The report always reports how many there are; this decides whether they are removed. Off by default."
      input: checkbox
      value: FALSE
   mask_imgt_position:
   ...
   sequenced_from_v_start:
      label: "`sequenced_from_v_start`: For protocols that sequence from the first nucleotide of V: drop sequences whose alignment does not start there. The report always shows how many there are; this decides whether they are removed. Off by default."
      input: checkbox
      value: FALSE
...
# Locus of the assigned genes

A library that targets one locus can still be handed a sequence assigned to
another: the aligner reports the best match in whatever reference it was given.
Such a sequence carries the wrong chain into duplicate collapsing and clonal
analysis, so the report states how many there are, per group.

```{r locus-counts}
# The locus column comes from the aligner when present; otherwise it is read off
# the V call, which is what getLocus() does. This report has no column parameters,
# so v_call is the AIRR field name.
# 'IG' or 'TR' name a whole class, a value like 'IGH' one locus.
wanted <- toupper(trimws(strsplit(params$locus, ",")[[1]]))

db <- db %>%
    mutate(locus_called = if ("locus" %in% colnames(db)) toupper(.data$locus)
                          else toupper(getLocus(.data$v_call)),
           # A sequence with no V call has no locus and matches neither, which is
           # what %in% gives for NA.
           in_locus = .data$locus_called %in% wanted |
                      substr(.data$locus_called, 1, 2) %in% wanted)

locus_tab <- db %>%
    group_by(!!!rlang::syms(unique(params$collapseby)), .data$locus_called, .data$in_locus) %>%
    summarize(sequences = n(), .groups = "drop")
```

```{r tab-locus, results='asis', echo=FALSE}
tab_locus <- eetable(locus_tab,
    caption = paste0("Sequences per assigned locus, and whether the locus is one of the targeted ones (",
...
```{r echo=FALSE, results='asis'}
print_table_caption(tag = "tab_locus", text = tab_locus$caption)
```

```{r filter-locus}
if (as.logical(params$restrict_to_locus)) {
    rows_in <- nrow(db)
    db <- db %>% filter(.data$in_locus)
    print(paste0("Filter out sequences whose locus is not one of ", paste(wanted, collapse = ", "), "."))
    print(paste0("Before filtering, there are ", rows_in, " sequences."))
    print(paste0("After filtering, there are ", nrow(db), " sequences."))
    if (nrow(db) == 0) {
        stop("'restrict_to_locus' removed all ", rows_in, " sequences: none of them were assigned to ",
             paste(wanted, collapse = ", "), ". Check that 'locus' names the loci these samples target.")
    }
}
```

```{r drop-locus-columns}
db <- db %>% select(-"locus_called", -"in_locus")
```

# Start of the V alignment

A protocol that targets the first nucleotide of V should produce alignments that
start at IMGT position 1. A sequence that starts later missed the 5' end of V:
its start cannot be trusted, and it carries a shorter V into duplicate collapsing
and clonal analysis. This is measured before any masking below, on the alignment
as it came from the aligner.

```{r v-start}
# Position of the first aligned nucleotide: -1 when the alignment is all gaps,
# NA when there is no alignment at all.
db <- db %>%
    mutate(v_start = as.integer(regexpr("[^.]", .data$sequence_alignment)),
           from_v_start = !is.na(.data$v_start) & .data$v_start == 1)

v_start_tab <- db %>%
    group_by(!!!rlang::syms(unique(params$collapseby))) %>%
    summarize(sequences = n(),
              from_v_start = sum(.data$from_v_start),
              .groups = "drop") %>%
    mutate(not_from_v_start = .data$sequences - .data$from_v_start)
```

```{r tab-v-start, results='asis', echo=FALSE}
tab_v_start <- eetable(v_start_tab,
    caption = "Sequences whose alignment starts at the first V nucleotide, and those that do not",
    outdir = params$outdir,
    file = "tab_v_start")
tab_v_start$table
```
...
```{r echo=FALSE, results='asis'}
print_table_caption(tag = "tab_v_start", text = tab_v_start$caption)
```

```{r p-v-start, fig.cap=p_v_start$enchantr$html_caption}
p_v_start <- ggplot(db,
                    aes(x = !!rlang::sym(unique(params$collapseby)[1]),
                        fill = from_v_start)) +
    geom_bar(linewidth = 0.2, color = "black") +
    coord_flip() +
    labs(fill = "starts at the\nfirst V nucleotide", y = "sequences") +
    theme_enchantr()
p_v_start <- eeplot(p_v_start, params$outdir, knitr::opts_current$get("label"),
                    caption = "Sequences per group, split by whether the alignment starts at the first V nucleotide.")
p_v_start
```
...
if (as.logical(params$sequenced_from_v_start)) {
    rows_in <- nrow(db)
    db <- db %>% filter(.data$from_v_start)
    print("Filter out sequences whose alignment does not start at the first V nucleotide.")
    print(paste0("Before filtering, there are ", rows_in, " sequences."))
    print(paste0("After filtering, there are ", nrow(db), " sequences."))
    if (nrow(db) == 0) {
        stop("'sequenced_from_v_start' removed all ", rows_in, " sequences. None of them ",
             "aligned from the first V nucleotide; check that this is the right protocol ",
             "for these data.")
    }
}
```

```{r drop-v-start-columns}
# The report's own columns, not part of the repertoire it passes on.
db <- db %>% select(-"v_start", -"from_v_start")
```

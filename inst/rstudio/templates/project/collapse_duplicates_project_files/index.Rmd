...
seq_locus <- if ("locus" %in% colnames(db)) {
    toupper(db[["locus"]])
} else {
    toupper(getLocus(db[["v_call"]]))
}
...
in_locus <- Reduce(`|`, lapply(wanted, function(w) {
    if (nchar(w) == 2) startsWith(seq_locus, w) else seq_locus == w
}), init = rep(FALSE, length(seq_locus)))

locus_tab <- db %>%
    mutate(locus_called = seq_locus, in_locus = in_locus) %>%
...
v_start <- as.integer(regexpr("[^.]", db[["sequence_alignment"]]))
from_v_start <- !is.na(v_start) & v_start == 1

v_start_tab <- db %>%
    mutate(from_v_start = from_v_start) %>%
...
p_v_start <- ggplot(db %>% mutate(from_v_start = from_v_start),
